 ...
 * 
 */
package com.iftikhar.spark.countryNameUpperCase;
 ...
 *
 */
public class CountryNameUpperCase {

	/**
	 * @param args
	 */
	static // Create a global delimiter for the file reader.
	...
	public static void main(String[] args) {
		...
		JavaPairRDD<String, String> upperCaseRDD = loadRDD(filePath, sc);

		// Before writing to a file check if the file already exists.
		checkIfFileExists(conf, sc, upperCaseRDD);

		// print RDD to the console.
		upperCaseRDD.collect().forEach(System.out::println);
	...
	private static JavaPairRDD<String, String> loadRDD(String filePath, JavaSparkContext sc) {
		...
		// Convert int pair RDD and change column 2 into upper case.
		JavaPairRDD<String, String> upperCaseCountryNameRDD = initialRDD.mapToPair(getFilterdRDD());

		// call getPairRDD method() and return the result to main.
		return upperCaseCountryNameRDD;
	} // end loadRDD.
	...
	private static PairFunction<String, String, String> getFilterdRDD() {
		// return a paired RDD in the form of Tuple2 by retrieving colum1 and column3
		// using split().
		return (PairFunction<String, String, String>) line -> new Tuple2<>(line.split(COMMA_DELIMITER)[1],
				line.split(COMMA_DELIMITER)[3].toUpperCase());
	} // end getFilterd().
	...
	private static void checkIfFileExists(SparkConf conf, JavaSparkContext sc, JavaPairRDD<String, String> printRDD) {
		...
		// File path holder.
		String filePath = "src/main/resources/contents/out/airports_uppercase.txt";
...
} // End Class CountryNameUpperCase.
